...
interface Logger {
    ...
    fun warn(tag: String, msg: String, vararg args: Any)
...
class TinyLogWrapper(logLocation: String?) : Logger {
    ...
    init {
        val config = Configurator
                .currentConfig()
                .formatPattern("{date:yyyy-MM-dd HH:mm:ss:SSSSSS} {thread} {level}: {message}")
                .level(Level.DEBUG)
    ...
    override fun warn(tag: String, msg: String, vararg args: Any) = Tiny.warn(prepareMsg(tag, msg), *args)
